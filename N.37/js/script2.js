...
function getClockSize(eo) {
	...
	if (clockSizeElem.value >= 200) {
		...
		// const hourArrowAngle = 0;
		/*
				const hourArrowX = clockCenterX + hourArrowLength * Math.sin(hourArrowAngle);
				const hourArrowY = clockCenterY - hourArrowLength * Math.cos(hourArrowAngle);
		
				const hourArrow = document.createElementNS('http://www.w3.org/2000/svg', 'line');
				hourArrow.setAttribute('x1', clockCenterX);
				hourArrow.setAttribute('y1', clockCenterY);
				hourArrow.setAttribute('x2', hourArrowX);
				hourArrow.setAttribute('y2', hourArrowY);
				hourArrow.setAttribute('stroke', 'black');
				hourArrow.setAttribute('stroke-width', 20);
				hourArrow.setAttribute('stroke-linecap', 'round');
		
				clockSvgBody.append(hourArrow);
		*/




		// buildArrow(clockSvgBody, hourArrowLength, hourArrowAngle, 20, arrowColor, clockCenterX, clockCenterY);
		...
		// const minArrowAngle = 0;

		/*
		const minArrowX = clockCenterX + minArrowLength * Math.sin(minArrowAngle);
		const minArrowY = clockCenterY - minArrowLength * Math.cos(minArrowAngle);

		const minArrow = document.createElementNS('http://www.w3.org/2000/svg', 'line');
		minArrow.setAttribute('x1', clockCenterX);
		minArrow.setAttribute('y1', clockCenterY);
		minArrow.setAttribute('x2', minArrowX);
		minArrow.setAttribute('y2', minArrowY);
		minArrow.setAttribute('stroke', 'black');
		minArrow.setAttribute('stroke-width', 8);
		minArrow.setAttribute('stroke-linecap', 'round');

		clockSvgBody.append(minArrow);
*/
		// buildArrow(clockSvgBody, minArrowLength, minArrowAngle, 8, arrowColor, clockCenterX, clockCenterY);
		...
		// const secArrowAngle = 0;

		// const secArrowX = clockCenterX + secArrowLength * Math.sin(secArrowAngle);
		// const secArrowY = clockCenterY - secArrowLength * Math.cos(secArrowAngle);

		// const secArrow = document.createElementNS('http://www.w3.org/2000/svg', 'line');
		// secArrow.setAttribute('x1', clockCenterX);
		// secArrow.setAttribute('y1', clockCenterY);
		// secArrow.setAttribute('x2', secArrowX);
		// secArrow.setAttribute('y2', secArrowY);
		// secArrow.setAttribute('stroke', 'black');
		// secArrow.setAttribute('stroke-width', 2);
		// secArrow.setAttribute('stroke-linecap', 'round');

		// clockSvgBody.append(secArrow);

		// buildArrow(clockSvgBody, secArrowLength, secArrowAngle, 2, arrowColor, clockCenterX, clockCenterY)

		//===============================================================================================
		// создаем цифровые часы
		const digitalClockDistance = clockRad * 0.32;	// смещение цифровых часов по оси Y
		const digitalClockFontSize = 40;		// задаем размер шрифта цифровым часам
		...
		function updateClock() {
			...
			while (clockSvgBody.querySelector('text')) {
				clockSvgBody.querySelector('text').remove();
			}
